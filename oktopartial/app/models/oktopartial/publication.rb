...
class Oktopartial::Publication < ActiveRecord::Base
  ...
  STATUS_CDS = [
    {:cd => '0', :key => :preparing        , :next_action => :prepare, :name => '公開準備中'    },
    {:cd => '1', :key => :preparation_error, :next_action => :prepare, :name => '公開準備エラー'},
    {:cd => '2', :key => :staging          , :next_action => :publish, :name => '公開待ち'      },
    {:cd => '3', :key => :publishing       ,                           :name => '公開中'        },
    {:cd => '4', :key => :published        ,                           :name => '公開済み'      },
    {:cd => '5', :key => :publishing_error , :next_action => :publish, :name => '公開エラー'    },
  ]
  ...
  STATUS_CD_TO_NEXT_ACTION = STATUS_CDS.inject({}){|d, entry| d[entry[:cd]] = entry[:next_action]; d}
  ...
  def next_action; STATUS_CD_TO_NEXT_ACTION[status_cd]; end
  ...
  def save_uploaded_file
    ...
    base_dir = Time.now.strftime('%Y%m%d%H%M%S') << '-' << File.basename(self.source_path, '.*')
    self.dirname = self.dest_dir ? File.join(base_dir, self.dest_dir) : base_dir
  end

  def clear_files
    FileUtils.rm_rf(actual_source_path) if File.exist?(actual_source_path)
    FileUtils.rm_rf(actual_dir_path) if File.exist?(actual_dir_path)
  end
  ...
  def publish!
    ...
    begin
      ...
      FileUtils.cp_r(actual_dir_path, dest_path)
      FileUtils.rm_rf(actual_dir_path)
      self.status_key = :published
    rescue Exception => error
      self.status_key = :publishing_error
      self.error_messages ||= ''
      self.error_messages << error.to_s << "\n  " << error.backtrace.join("\n  ")
    end
    self.save!
  ...
  def dest_path_in_public
    dummy, path = self.dirname.gsub(/^\/|\/$/, '').split(/\//, 2)
    path
  end

  def actual_dest_path
    File.join(RAILS_ROOT, 'public', dest_path_in_public)
  end


  def entry_paths
    if archive? && [:preparation_error, :staging].include?(self.status_key)
      result = []
